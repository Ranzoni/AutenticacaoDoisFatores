...
{
    public class RemoverPermissoesParaUsuario(DominioDePermissoes dominio, DominioDeUsuarios usuarios, INotificador notificador)
    {
        ...
        public async Task ExecutarAsync(Guid idUsuario, IEnumerable<TipoDePermissao> permissoes)
        {
            ...
            if (!RemocaoEhValida(usuario))
                return;

            await _dominio.RemoverAsync(idUsuario, permissoes);
        }

        private bool RemocaoEhValida(Usuario? usuario)
        {
